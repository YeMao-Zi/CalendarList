...
import { festival, lFestival } from "./festival-data";
 ...
 * @returns 当月的日历信息
 */
const getDateTimeList = (year: number, month: number) => {
  ...
  let firstDaysWeek =
    dayjs(currentMonth).startOf("month").day() === 0
      ? 7
      : dayjs(currentMonth).startOf("month").day(); // 1
  // 这个值是个闭包，一直传到 getDateInfo 函数中进行累加
  let dateIndex = 0;
  // 处理上个月日期信息
  const lastMonthDays = lastMonth.daysInMonth(); // 30
  const startIndex = lastMonthDays - firstDaysWeek; // 29

  for (
    let lastDayIndex = startIndex + 1;
